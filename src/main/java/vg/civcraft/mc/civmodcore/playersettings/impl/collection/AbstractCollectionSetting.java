...
public abstract class AbstractCollectionSetting<C extends Collection<T>, T> extends PlayerSetting<C> {
    ...
    public AbstractCollectionSetting(JavaPlugin owningPlugin, C defaultValue, String name, String identifier, ItemStack gui, String description, Class<T> elementClass, Function<C, C> newFunction) {
        ...
        if (elementSetting == null) {
            throw new IllegalArgumentException("Can not keep " + elementClass
                    .getName() + " in collection, because it was not registed in SettingTypeManager");
        }
    }
        ...
        for (T element : value) {
            ...
            clickables.add(new Clickable(is) {
                ...
                public void clicked(Player p) {
                    ...
                    player.sendMessage(
                            String.format("%sRemoved %s from %s", ChatColor.GREEN, elementSetting.toText(element),
                                    getNiceName()));
