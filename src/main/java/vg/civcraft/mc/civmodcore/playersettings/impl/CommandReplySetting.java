...
public class CommandReplySetting extends StringSetting {
    ...
    public CommandReplySetting(JavaPlugin plugin, String defaultValue, String name, String identifier, ItemStack gui, String description) {
    ...
    public ItemStack getGuiRepresentation(UUID player) {
        ...
        for (CommandArgument cArg : exampleArguments.values()) {
            ...
            ItemAPI.addLore(item,
                    ChatColor.YELLOW + "%%" + cArg.identifier + "%% will be replaced with " + cArg.description);
        }
        return item;
    }
