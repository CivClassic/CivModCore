...
public class StandaloneCommandHandler {
    ...
    public List<String> tabCompleteCommand(CommandSender sender, org.bukkit.command.Command cmd, String[] args) {
        ...
        if (command == null) {
            plugin.getLogger()
                    .warning("Could not tab complete command " + cmd.getName() + ", no implementation was provided");
        ...
        if (sender instanceof Player) {
            ...
            if (command.isRateLimitedToTabComplete((Player) sender)) {
                sender.sendMessage(
                        ChatColor.RED + "You are rate limited and have to wait before tab completing this command again");
                return Collections.emptyList();
            }
        }
        else {
    ...
    private void loadAll() {
        ...
        @SuppressWarnings("deprecation") JavaPluginLoader pluginLoader = new JavaPluginLoader(Bukkit.getServer());
    ...
    private void loadCommand(String classPath, PluginDescriptionFile pluginYml) {
        ...
        if (commandSection == null) {
            plugin.getLogger().warning("No command with the identifier " + command
                    .getIdentifier() + " could be found in the plugin.yml. Command will be unavailable");
            return;
        }
        @SuppressWarnings("unchecked") Map<String, Object> commandMap = (Map<String, Object>) commandSection;
        ...
        if (consoleOnly != null) {
            ...
            if (consoleOnly && playerOnly != null && playerOnly) {
                plugin.getLogger().severe("Command " + command
                        .getIdentifier() + " is simultaneously console only and player only. It can not be run");
            }
        }
