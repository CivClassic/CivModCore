    ...
    public Collection<Chunk> getChunks() {
        ...
        for (double x = center.getX() - xSize; x <= center.getX() + xSize; x += 16) {
            for (double z = center.getZ() - zSize; z <= center.getZ() + zSize; z += 16) {
                ...
                if (isInArea(new Location(c.getWorld(), c.getX() * 16, 0, (c.getZ() * 16) + 15)) || isInArea(new Location(c.getWorld(), c.getX() * 16, 0, c.getZ() * 16)) || isInArea(new Location(c.getWorld(), (c.getX() * 16) + 15, 0, c.getZ() * 16)) || isInArea(new Location(c.getWorld(), (c.getX() * 16) + 15, 0, (c.getZ() * 16) + 15))) {
                    chunks.add(c);
                }
            }
        }
