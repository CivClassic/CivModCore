...
public class ConfigParsing {
    ...
    public static ItemMap parseItemMapDirectly(ConfigurationSection current) {
        ...
        try {
            ...
            if (m == null) {
                log.severe("Failed to find material " + current.getString("material") + " in section " + current
                        .getCurrentPath());
                return im;
            }
        }
        ...
        if (meta == null) {
            ...
            if (current.contains("enchants")) {
                for (String enchantKey : current.getConfigurationSection("enchants").getKeys(false)) {
                    ...
                    if (!enchantConfig.isString("enchant")) {
                        log.warning("No enchant specified for enchantment entry at " + enchantConfig
                                .getCurrentPath() + ". Entry was ignored");
                        continue;
                    }
                    Enchantment enchant;
                    enchant = Enchantment
                    ...
                    if (enchant == null) {
                        log.severe("Failed to parse enchantment " + enchantConfig
                                .getString("enchant") + ", the entry was ignored");
                        continue;
                    }
            ...
            if (m == Material.POTION || m == Material.SPLASH_POTION || m == Material.LINGERING_POTION || m == Material.TIPPED_ARROW) {
                ...
                if (potion != null) {
                    ...
                    try {
                        ...
                        log.warning("Expected potion type at " + potion.getCurrentPath() + ", but " + potion
                                .getString("type") + " is not a valid potion type");
    ...
    public static List<PotionEffect> parsePotionEffects(ConfigurationSection configurationSection) {
        ...
        if (configurationSection != null) {
            for (String name : configurationSection.getKeys(false)) {
                ...
                if (type == null) {
                    log.severe("Expected potion type to be specified, but found no \"type\" option at " + configEffect
                            .getCurrentPath());
                    continue;
                }
                PotionEffectType effect = PotionEffectType.getByName(type);
                if (effect == null) {
                    log.severe("Expected potion type to be specified at " + configEffect
                            .getCurrentPath() + " but found " + type + " which is no valid type");
                }
     ...
     */
    public static <K, V> void parseKeyValueMap(ConfigurationSection parent, String identifier, Logger logger, Function<String, K> keyConverter, Function<String, V> valueConverter, Map<K, V> mapToUse) {
        ...
        for (String keyString : section.getKeys(false)) {
            ...
            try {
                ...
                logger.warning(
                        "Failed to parse " + identifier + " " + keyString + " at " + section.getCurrentPath() + ": " + e
                                .toString());
                continue;
            }
