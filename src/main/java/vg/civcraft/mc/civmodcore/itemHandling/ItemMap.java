...
public class ItemMap {
    ...
    public ItemMap getStacksByMaterialDurabilityEnchants(Material m, int durability, Map<Enchantment, Integer> enchants) {
        ItemMap result = new ItemMap();
        for (ItemStack is : items.keySet()) {
            if (is.getType() == m && is.getDurability() == durability && is.getItemMeta() != null && is.getItemMeta().getEnchants().equals(enchants)) {
                result.addItemAmount(is.clone(), items.get(is));
            }
        }
        return result;
    ...
    public ItemMap getStacksByMaterialDurabilityEnchants(ItemStack is) {
        if (is.getItemMeta() != null) {
            return getStacksByMaterialDurabilityEnchants(is.getType(), is.getDurability(), is.getItemMeta().getEnchants());
        }
        else {
            return getStacksByMaterialDurabilityEnchants(is.getType(), is.getDurability(), new HashMap<Enchantment, Integer>());
        }
    }
    ...
    public int getAmount(ItemStack is) {
        ...
        for (Entry<ItemStack, Integer> entry : matSubMap.getEntrySet()) {
            ...
            if ((is.getDurability() == -1 || is.getDurability() == current.getDurability()) && is.getItemMeta().equals(current.getItemMeta())) {
    ...
    private static ItemStack createMapConformCopy(ItemStack is) {
        ...
        if (s == null) {
            log.info("Attempted to create map conform copy of " + copy.toString() + ", but couldn't because this item can't be held in inventories since Minecraft 1.8");
            return null;
        }
        s.setRepairCost(0);
