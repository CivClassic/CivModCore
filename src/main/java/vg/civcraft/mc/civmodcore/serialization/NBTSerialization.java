...
public final class NBTSerialization {
    ...
    private static final Logger logger = Logger.getLogger(NBTSerialization.class.getSimpleName());
     ...
     *         public zero argument constructor, or 3) is already registered. (Aliases that are already registered will
     *         print a warning in lieu of throwing.)
     */
    public static <T extends NBTSerializable> void registerNBTSerializable(Class<T> clazz, String... aliases) {
        if (clazz == null) {
            throw new IllegalArgumentException("Cannot register NBTSerializable: " + "the given class is null.");
        }
        ...
        if (aliases != null) {
            ...
            for (String alias : aliases) {
                if (Strings.isNullOrEmpty(alias)) {
                    logger.warning(errorMessage + "the alias is null or empty.");
                    continue;
                }
                if (alias.equals(clazz.getName())) {
                    logger.warning(errorMessage + "the alias matches the class name.");
                    continue;
                }
                if (registeredClasses.containsKey(alias)) {
                    logger.warning(errorMessage + "the alias is already registered.");
                    continue;
                }
    ...
    public static <T extends NBTSerializable> void unregisterNBTSerializable(Class<T> clazz) {
        if (clazz == null) {
            throw new IllegalArgumentException("Cannot unregister NBTSerializable: " + "the given class is null.");
        }
