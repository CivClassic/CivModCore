...
final class NodeIterator<NodeType extends BaseOcTreeNode<NodeType, ValueType>, ValueType> implements Iterator<NodeType> {
	...
	void pushChildren(NodeType next) {
		if (next.hasChildren()) {
			...
			for (int i = 0; i < VolumeOcTreeNode.CHILDREN_SIZE; i++) {
