 ...
 */
public class BlockBasedChunkMetaView<T extends BlockBasedChunkMeta<D, S>, D extends BlockDataObject<D>, S extends BlockBasedStorageEngine<D>> extends ChunkMetaView<T> {
    ...
    BlockBasedChunkMetaView(JavaPlugin plugin, short pluginID, GlobalChunkMetaManager globalManager, Supplier<T> chunkProducer, boolean loadAll, boolean allowAccessUnloaded) {
        ...
        if (chunk == null) {
            ...
            if (data == null) {
                return storageEngine
                        .getForLocation(location.getBlockX(), location.getBlockY(), location.getBlockZ(), worldID,
                                pluginID);
            }
            return data;
        }
    ...
    public void postLoad(ChunkMeta<?> c) {
        @SuppressWarnings("unchecked") T chunk = (T) c;
