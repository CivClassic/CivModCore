...
public class SparseQuadTree {
        ...
        if (boxes_ != null) {
            ...
            if (includeBorder) {
                for (QTBox box : boxes_) {
                    if (box.qtXMin() - border <= x && box.qtXMax() + border >= x && box.qtZMin() - border <= y && box.qtZMax() + border >= y) {
                        result.add(box);
                    }
                }
            }
    ...
    protected void split() {
        ...
        if (nw_.size() == boxes_.size() || sw_.size() == boxes_.size() || ne_.size() == boxes_.size() || se_.size() == boxes_.size()) {
