...
public class SparseQuadTree<T extends QTBox> {
    ...
    public Set<T> find(int x, int z, boolean includeBorder) {
        ...
        if (boxes != null) {
            ...
            if (includeBorder) {
                for (T box : boxes) {
                    if (box.qtXMin() - border <= x && box.qtXMax() + border >= x && box.qtZMin() - border <= z && box
                            .qtZMax() + border >= z) {
                        result.add(box);
                    }
                }
    ...
    protected void split() {
        ...
        if (northWest.size() == boxes.size() || southWest.size() == boxes.size() || northEast.size() == boxes
                .size() || southEast.size() == boxes.size()) {
